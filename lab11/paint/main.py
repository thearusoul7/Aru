...
# Инструменты
TOOLS = ["Pencil", "Line", "Rectangle", "Circle", "Square",
...
def draw_preview(surface, tool, start, end, color, size):
    ...
    if tool == "Line":
        ...
        size_side = min(abs(end[0] - start[0]), abs(end[1] - start[1]))
        x = start[0]
        y = start[1]
        pygame.draw.rect(surface, color, (x, y, size_side, size_side), size)
...
def commit_shape(canvas_surf, tool, start, end, color, size):
    ...
    if tool == "Line":
        ...
        size_side = min(abs(end[0] - start[0]), abs(end[1] - start[1]))
        pygame.draw.rect(canvas_surf, color,
                         (start[0], start[1], size_side, size_side), size)

    elif tool == "Right Triangle":
        points = [start, (end[0], start[1]), end]
        pygame.draw.polygon(canvas_surf, color, points, size)
